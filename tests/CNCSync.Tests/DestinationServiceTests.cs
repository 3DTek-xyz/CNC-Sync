...
public sealed class DestinationServiceTests
{
    ...
    [Fact]
    public async Task RemoteDestination_WithDirectCheckEnabled_SkipsVpnWhenDestinationIsReachable()
    {
        var ftpService = new TrackingFtpService();
        var vpnService = new StubVpnService();
        var destination = new DestinationSettings
        {
            ...
            Host = "example.local",
            RequiredVpnConnectionName = "Office VPN",
            CheckDirectConnectionBeforeVpn = true
        };
        ...
        Assert.True(result.Success);
        Assert.Equal(2, ftpService.TestCallCount);
        Assert.Null(vpnService.LastEnsuredConnectionName);
        Assert.DoesNotContain("Connected required VPN", result.Message, StringComparison.Ordinal);
    }
    ...
    public async Task TestConnectionAsync_DisconnectsVpnAfterIdleTimeoutWhenConfigured()
    {
        ...
        Assert.Equal("Office VPN", vpnService.LastDisconnectedConnectionName);
    }
