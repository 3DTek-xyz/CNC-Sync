...
public sealed class SyncCoordinator : ISyncCoordinator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var appendProcessedFolderName = ShouldAppendProcessedFolderName(result, profile);
        var effectiveRemotePath = appendProcessedFolderName
            ? CombineRemoteDirectoryPath(remoteDirectoryPath, result.RemoteFolderName)
            : remoteDirectoryPath;

        if (destination.ReplaceRemoteFolderOnUpload &&
            appendProcessedFolderName &&
            !string.IsNullOrWhiteSpace(effectiveRemotePath))
        {
            ...
            {
                LogActivity($"Removed previous remote folder contents at {effectiveRemotePath}.", profile.Name);
            }
            else
            {
        ...
        var progressSource = string.IsNullOrWhiteSpace(destination.Name)
            ? profile.Name
            : $"{profile.Name} -> {destination.Name}";
        var progressKey = $"{profile.Id}:{destination.Id}";
        var progress = new Progress<string>(message => LogActivity(message, progressSource, isProgressUpdate: true, progressKey: progressKey));
        var uploadResult = await _destinationService.UploadFileSystemItemAsync(result.OutputPath, destination, effectiveRemotePath, progress, cancellationToken);
    ...
    private void LogActivity(string message, string? source = null, bool isProgressUpdate = false, string? progressKey = null) =>
        ActivityLogged?.Invoke(new ActivityLogEntry
        {
            ...
            ProgressKey = progressKey ?? string.Empty
        });
