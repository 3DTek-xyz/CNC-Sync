...
public sealed class DestinationService : IDestinationService
{
    ...
    public async Task<(bool Success, string Message)> UploadDirectoryAsync(string localPath, DestinationSettings destination, string remoteDirectoryPath, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        return await UploadFileSystemItemAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken);
    }

    public async Task<(bool Success, string Message)> UploadFileSystemItemAsync(string localPath, DestinationSettings destination, string remoteDirectoryPath, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        var vpnResult = await EnsureVpnConnectedIfRequiredAsync(destination, cancellationToken);
        if (!vpnResult.Success)
        {
        ...
        try
        {
            var result = destination.Type switch
            {
                DestinationType.LocalFolder => await UploadToLocalFolderAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken),
                DestinationType.NetworkShare => await _networkShareService.UploadFileSystemItemAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken),
                DestinationType.Sftp => await _sftpService.UploadFileSystemItemAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken),
                DestinationType.Scp => await _scpService.UploadFileSystemItemAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken),
                _ => await _ftpService.UploadFileSystemItemAsync(localPath, destination, remoteDirectoryPath, progress, cancellationToken)
            };
    ...
    private async Task<VpnConnectionEnsureResult> EnsureVpnConnectedIfRequiredAsync(DestinationSettings destination, CancellationToken cancellationToken)
    {
        ...
        if (destination.CheckDirectConnectionBeforeVpn &&
            await CanReachDestinationDirectlyAsync(destination, cancellationToken))
        {
            return VpnConnectionEnsureResult.NoRequirement();
        }
    ...
    private async Task<bool> CanReachDestinationDirectlyAsync(DestinationSettings destination, CancellationToken cancellationToken)
    {
        try
        {
            var result = destination.Type switch
        ...
        try
        {
            ...
            else
            {
                ...
                for (var index = 0; index < files.Count; index++)
                {
                    ...
                    progress?.Report($"Copying {index + 1}/{files.Count}: {relativePath.Replace('\\', '/')}");
