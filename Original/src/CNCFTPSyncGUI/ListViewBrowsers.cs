...
namespace CNCFTPSyncGUI
{
    public class ListViewBrowser : ListView
    {
                ...
                using (var folderDialog = new FolderBrowserDialog())
                {
                    ...
                    if (folderDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (mainForm != null)
                        {
                            ...
                            if (ftpServiceField?.GetValue(mainForm) is CNCFTPSyncCore.Services.FtpService ftpService &&
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    if (mainForm != null)
                    {
                        ...
                        if (ftpServiceField?.GetValue(mainForm) is CNCFTPSyncCore.Services.FtpService ftpService && 
                            configField?.GetValue(mainForm) is CNCFTPSyncCore.Models.SyncConfiguration config &&
                            currentPathField?.GetValue(mainForm) is string currentPath)
                        {
        ...
        private async Task<bool> UploadFolderRecursive(CNCFTPSyncCore.Services.FtpService ftpService, string localPath, string remotePath)
        {
            try
            {
